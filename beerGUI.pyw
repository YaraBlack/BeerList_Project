...
class AttentionMessageBox(QMessageBox):
    ...
    def invokeMsgBox(self, msgType: int):
        match msgType:
            ...
            case 9:
                self.setText(
                    "That beer is already in the list!\n\
...
class RegistrationForm(QDialog):
    def __init__(self):
        ...
        self.reg_layout.addWidget(
            self.button_register, 1, Qt.AlignmentFlag.AlignHCenter
        )
    ...
    def regSubmit(self):
        if self.input_regLogin.text() and self.input_regPass.text():
            ...
            data = {
                "username": self.input_regLogin.text(),
...
class MainWindow(QMainWindow):
    ...
    def clearEdit(self):
        ...
        self.imagePath = ""
        self.addForm_layout.addWidget(
            self.img_placeholder, 0, 0, 1, 5, Qt.AlignmentFlag.AlignHCenter
        )
    ...
    def setImage(self, source, sourcePath=""):
        ...
        if path:
            ...
            self.addForm_layout.addWidget(
                self.label_image, 0, 0, 1, 5, Qt.AlignmentFlag.AlignHCenter
            )
        else:
            print("Operation has been canceled!")
    ...
    def checkFloatInput(self):
        ...
        errCounter = 0
        if not (
            self.input_beerName.text()
            and self.input_beerAlco.text()
            and self.input_beerPoints.text()
            and self.input_beerIngr.toPlainText()
            and self.input_beerTaste.toPlainText()
        ):
            self.AttMessageBox.invokeMsgBox(1)
            errCounter += 1
        elif len(alcCheck) > 1:
    ...
    def addItemToTable(self, state: str):
        if self.checkFloatInput():
            if state == "new":
                ...
                self.tempDict = dict(
                    list(self.tempDict.items())[: self.activeRow]
    ...
    def addNew(self):
        ...
        self.button_confirmForm.clicked.connect(
            partial(self.addItemToTable, "new")
        )
    ...
    def editItem(self):
        ...
        self.button_confirmForm.clicked.disconnect()
        self.button_confirmForm.clicked.connect(
            partial(self.addItemToTable, "edit")
        )
    ...
    def getDataFromTable(self, item: QTableWidgetItem):
        ...
        if item.column() == 0:
            if item.data(0) in self.tempDict:
                ...
                self.input_beerAlco.setText(
                    f"{self.tempDict[item.data(0)]['alco']}"
                )
                self.input_beerPoints.setText(
                    f"{self.tempDict[item.data(0)]['grade']}"
                )
                self.input_beerIngr.setPlainText(
                    f"{self.tempDict[item.data(0)]['ingr']}"
                )
                self.input_beerTaste.setPlainText(
                    f"{self.tempDict[item.data(0)]['descr']}"
                )
                if self.tempDict[item.data(0)]["imgPath"]:
                    self.setImage(
                        "func", self.tempDict[item.data(0)]["imgPath"]
                    )
                else:
                    self.addForm_layout.addWidget(
...
{self.input_username.text()}!"
    ...
    def createContent(self):
        ...
        self.main_contentFrame.setStyleSheet(
            "QFrame \
        ...
        self.edit_contentFrame.setStyleSheet(
            "QFrame \
        ...
        self.controlPanel_contentFrame.setStyleSheet(
            "QFrame \
                                        { border-bottom: 1px solid black; }"
        )

        # Control panel section Layout
        self.controlPanel_contentLayout = self.gridTemplate(
            self.controlPanel_contentFrame
        )
        self.controlPanel_contentFrame.setLayout(
            self.controlPanel_contentLayout
        )

        # Add buttons to the control panel
        self.button_addNew = QPushButton(
            "Add New", self.controlPanel_contentFrame
        )
        self.button_addNew.setFixedSize(70, 25)

        self.button_remove = QPushButton(
            "Remove", self.controlPanel_contentFrame
        )
        self.button_remove.setFixedSize(70, 25)
        self.button_remove.setEnabled(False)

        self.button_editContent = QPushButton(
            "Edit", self.controlPanel_contentFrame
        )
        ...
        self.controlPanel_contentLayout.addWidget(
            self.button_editContent, 0, 5
        )
        ...
        self.inputAndInfo_contentFrame.setStyleSheet(
            "QFrame \
                                        {background-color: orange;}"
        )

        # # Input and info section Layout
        inputAndInfo_contentLayout = self.gridTemplate(
            self.inputAndInfo_contentFrame
        )
        ...
        self.addForm_layout.addWidget(
            self.button_addImage, 1, 0, 1, 5, Qt.AlignmentFlag.AlignHCenter
        )
        ...
        self.addForm_layout.addWidget(
            self.button_confirmForm, 6, 0, 1, 5, Qt.AlignmentFlag.AlignHCenter
        )
        ...
        self.table.setHorizontalHeaderLabels(
            ["Name", "Alc %", "Rating", "Ingridients", "Description"]
        )
        # self.table.setModel()
